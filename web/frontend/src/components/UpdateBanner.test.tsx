...
const { mockedToast } = vi.hoisted(() => ({ mockedToast: vi.fn() }))
...
describe('UpdateBanner — service-worker update banner', () => {
  ...
  it('fires a persistent toast after controllerchange', async () => {
    stubFetch('1.14.0', null)
    renderBanner()
    fireSWControllerChange()
    await waitFor(() =>
      expect(mockedToast).toHaveBeenCalledWith(
        'App updated in background',
        expect.objectContaining({ duration: Infinity }),
      ),
    )
  })

  it('toast includes a Reload action', async () => {
    stubFetch('1.14.0', null)
    renderBanner()
    fireSWControllerChange()
    await waitFor(() =>
  ...
  it('fires toast only once even if controllerchange fires twice', async () => {
    stubFetch('1.14.0', null)
    renderBanner()
    fireSWControllerChange()
    fireSWControllerChange()
    await waitFor(() =>
      expect(mockedToast).toHaveBeenCalledOnce(),
    )
  })
})
