        ...
        return (
          <div key={key} className="space-y-1">
            ...
            {def.type === 'object' && def.properties ? (
              /* Nested object — render each sub-property inline */
              <div className="space-y-2 pl-2 border-l-2 border-border">
                {Object.entries(def.properties as Record<string, SchemaProperty>).map(([subKey, subDef]) => {
                  const subValue = ((value as Record<string, unknown> | undefined) ?? {})[subKey]
                  const subLabel = subDef.title ?? subKey
                  const subHint = subDef.description
                  return (
                    <div key={subKey} className="flex items-start gap-2">
                      {subDef.type === 'boolean' ? (
                        <button
                          ...
                          aria-label={`Toggle ${subLabel}`}
                          onClick={() => {
                            const parent = (value as Record<string, unknown> | undefined) ?? {}
                            onChange(key, { ...parent, [subKey]: !subValue })
                          }}
                          className={cn(
                            'flex-shrink-0 w-8 h-4 rounded-full transition-colors mt-0.5',
                            subValue ? 'bg-primary' : 'bg-border',
                          )}
                        >
                          <span
                            className={cn(
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            const parent = (value as Record<string, unknown> | undefined) ?? {}
                            onChange(key, { ...parent, [subKey]: e.target.value })
                          }}
...
function PluginRow({
  ...
  return (
    <AccordionItem value={plugin.name} className="border-b border-border last:border-0 overflow-hidden">
      <AccordionTrigger className="py-3 hover:no-underline [&>svg]:ml-2 overflow-hidden">
        <div className="flex items-center gap-3 flex-1 min-w-0 text-left overflow-hidden">
          ...
          <div className="flex-1 min-w-0 overflow-hidden">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-sm font-medium text-foreground truncate">{plugin.display_name}</span>
        ...
        <div className="space-y-2">
          {filtered.map((pkg) => {
            ...
            return (
              <div
                key={pkg.pypi}
                className="flex items-start gap-3 py-2 border-b border-border last:border-0 overflow-hidden"
              >
...
export function PluginsPage() {
  ...
  // Core plugins (e.g. content_filter) have dedicated Settings sections —
  // hide them from the Plugins page so only installable plugins appear.
  const plugins = (data?.plugins ?? []).filter((p) => p.tier !== 'core')
    ...
    <Layout>
      <div className="flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden overscroll-contain">
          <div className="max-w-2xl mx-auto px-4 py-6 space-y-8 overflow-hidden">
